        ...
        ref.child("users").observeSingleEvent(of: .value, with: { snapshot in
            ...
            for (userId, userInfo) in usersDict {
                guard userId != currentUserId else { continue }
                      let username = userInfo["username"] as? String ?? "Anonymous"

                let chatId = self.generateChatId(user1: currentUserId, user2: userId)
                group.enter()

                self.ref.child("chats").child(chatId).observeSingleEvent(of: .value, with: { chatSnapshot in
                    ...
                    if let lastMessage = messages.sorted(by: { $0.time > $1.time }).first {
                        ...
                        let userChat = ListUserChat(id: UUID().uuidString, listUserId: userId, username: username, lastMessage: nil)
                        fetchedChats.append(userChat)
                    }
            ...
            group.notify(queue: .main) {
                self.userChats = fetchedChats.sorted(by: {
                    // Sort nil lastMessage to the bottom
                    ($0.lastMessage?.time ?? .distantPast) > ($1.lastMessage?.time ?? .distantPast)
                })
            }
        })
    }
